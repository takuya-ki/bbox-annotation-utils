    ...
    def get_bbox(self, img_shape):
        ...
        for object_tree in self.root.findall('object'):
            ...
            if height!=img_height or width!=img_width:
                ...
                print("Bounding box positions of the ground truth are modified. (Not sure if this always does work.)")
...
def draw_annotation(image_paths, xml_dirpath):
    ...
    for i, image_path in enumerate(image_paths):
        ...
        if not os.path.isfile(xml_path):
            print(xml_path + " is not existed, so this process will be continued.")
            exit()
        ...
        for i, (label, bbox) in enumerate(zip(labels, bboxs)):
            ...
            cv2.putText(drawed, label, text_pos, cv2.FONT_HERSHEY_PLAIN, 3.5, c, 5)
            cv2.rectangle(drawed, (int(bbox[2]),int(bbox[3])), (int(bbox[0]),int(bbox[1])), c, 2)
            cv2.circle(drawed,(int(bbox[2]),int(bbox[3])), 10, c, -1)
            cv2.circle(drawed,(int(bbox[0]),int(bbox[3])), 10, c, -1)
            cv2.circle(drawed,(int(bbox[0]),int(bbox[1])), 10, c, -1)
            cv2.circle(drawed,(int(bbox[2]),int(bbox[1])), 10, c, -1)

        cv2.imshow("drawed", scale_to_width(drawed, 1000))
        cv2.imwrite(os.path.join(
            dataset_dirpath,'result',os.path.splitext(os.path.basename(image_path))[0]+'.'+imgext),drawed)
...
if __name__ == '__main__':
    ...
    imgext = 'jpg'
    dataset_dirpath = os.path.join(
        os.path.dirname(__file__), "..", "dataset")
    image_paths = glob.glob(os.path.join(
        dataset_dirpath, imgext, '*.'+imgext))
    xml_dirpath = os.path.join(dataset_dirpath, "xml")
